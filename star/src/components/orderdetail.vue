<template>
	...
	<div class="wait" v-if='token'>
		<!--<div class="linkman">
			<p>联系人信息</p>
			<div>
				<span>姓名:{{detailInfo.contacts}}</span>
				<span>手机号码:{{detailInfo.contactsPhone}}</span>
			</div>
		</div>-->
		
		<div class="linkman">
			<p>上课子女信息</p>
			<div v-if='detailInfo.children'>
...
<script>
	...
	export default {
		...
		methods: {
			topay() {
				window.location.href = '/#/orderpay/' + this.ordernum
				//				this.$router.push('/orderpay/' + this.ordernum)
			},
			cancelorder() {
				let $this = this
