<template>
	<div class="newsdetail-box">
		...
		<div class="text">
			...
			<p class="subhead">
				{{newsinfo.createTime}}<span>阅读:{{newsinfo.readCount}}</span>
			</p>
		...
		<div class="readnum">
			...
			<div><img @click="upVote" src="../../static/img/upvote.png"><span>{{newsinfo.upVoteCount}}</span></div>
		</div>
		...
		<div class="pl">
			...
			<ul class="commnet-list">
				<li v-for="comment in commentList">
					...
					<div class="content">
						<p><span>{{comment.userVo.nickname}}</span> <em>{{comment.createTime.substr(0,10)}}</em></p>
...
<script>
	...
	export default {
		...
		//			sessionStorage.setItem('paths', from.path)
		//			next()
		//		},
		//		beforeRouteLeave(to, from, next) {
		//			if(sessionStorage.getItem('paths') === to.path) {
		//				this.$vux.alert.show({
		//					title: '提示',
		//					content: this.title,
		//					onHide() {
...
//			)
			...
			//			that.$http.post(this.lchttp + '/app/prompt/get').then(
			//				(res) => {
			//					this.title = res.data.obj.content
