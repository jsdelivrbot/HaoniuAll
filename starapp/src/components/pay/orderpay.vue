	...
	export default {
		...
		methods: {
			alipay() {
				...
				//				})
				//
				//				var ALIPAYSERVER = '/ali/pay?outTradeNo=' + this.ordernum
				//				var WXPAYSERVER = '/weixin/pay?outTradeNo=' + this.ordernum
				//
				//				function pay(id) {
				//					var PAYSERVER = ''
				//					if(id === 'alipay') {
				...
				//					}
				//
				//					$this.$http.get(PAYSERVER).then(
				//						(res) => {
				//							if($this.selectPay === 2) {
				//								if(res.data.success) {
				//									plus.payment.request(channel, res.data.result, function(result) {
				//										plus.nativeUI.alert('支付成功！', function() {
				//											$this.$router.replace('/successpay/' + $this.ordernum)
				//										})
				//									}, function(error) {
