	...
	<div class="searchlist">
		<div class="search-box vux-1px-b">
			<div class="out-img" @click="goback()">
				<img src="../../../static/img/../../static/img/outleft.png" />
			</div>

			<div class="input">
				<img src="../../../static/img/../../static/img/iconsearch.png" />
				<input type="search" v-model="val" name="" @keydown.enter="search()" id="" value="" placeholder="请输入您想要搜索的内容" />
			</div>
			<span @click="search(1)">搜索</span>
		</div>

		<div class="select-box">
			<ul class="select-list vux-1px-b">
			...
			<div class="select-detail" style="min-height: calc(100% - 88px); " v-show='tabbar>0'>
				<picker :data='kecheng' :columns=1 @on-change='search(-1)' v-model='selectvalue1' style='background: #fff;' v-show='tabbar==1'></picker>
		...
		<courseList v-if='selectvalue1[0]!=="机构"' style='margin-top: 94px;' :data='list' :iscollect='1'></courseList>
		<jigoulist v-if='selectvalue1[0]=="机构"' style='margin-top: 94px;' :list='jigoulist'></jigoulist>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.$http.get('/search?page=1&rows=100&' + this.$route.params.name).then(
				(res) => {
		...
		methods: {
			...
			search2() {
				...
				this.$http.get('/search', {
					params: {
						keyword: '',
						target: 'k',
						cat: searchtxt.substring(0, searchtxt.length - 1),
						page: '1',
						rows: '100',
						lng: localStorage.getItem('lng'),
						lat: localStorage.getItem('lat'),
			...
			search3() {
				...
				//				alert(this.selectvalue3[0])
				this.$http.get('/search', {
					params: {
						keyword: '',
						target: 'k',
						age: this.selectvalue3[0],
						page: '1',
						rows: '100',
						lng: localStorage.getItem('lng'),
						lat: localStorage.getItem('lat'),
		...
		.select-box {
			...
			.select-list {
				...
				li {
					...
					font-size: 14px;
					color: #999;
					background: url(../../../static/img/../../static/img/selectdown1.png) no-repeat;
