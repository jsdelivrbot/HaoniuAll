<template>
	<div>
		<div class="kcb-topbar">
			<em></em>
			<ul>
				<li v-for="item in childrenList" @click="getTimeline(item.id)" :class="{'tips':item.tixin,'active':changeChildrenSelect==item.id}">
					{{item.name}}
				</li>
			</ul>
			<em></em>
		...
		<div class="timeline" ref='bsc' :style="{height:bsheight+'px'}" style="position: fixed;top: 44px;bottom: 54px;width: 100%;">
			<ul class="timeline-list" ref='ht'>
				<li v-for="item in timeline" :class="{'active-two':item.xing==2,'active-today':item.xing==0,'active-before':item.xing==1}">
					<div class="left-line">
		...
		methods: {
			...
			bs() {
				...
				this.scroll.on('touchend', (pos) => {
					...
					//					if((this.$refs.ht.offsetHeight - this.$refs.bsc.offsetHeight) < (pos.y * -1) && this.laterEnd) {
					//						this.downTimeline()
					//						return false
					//					}
				})
