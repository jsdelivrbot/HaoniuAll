<template>
	<div style="height: calc(100vh-88px);">
		<baidu-map class='map' :zoom='zoom' v-if='center' @ready="handler" :center='centers'>
			...
			<bm-label v-for='(item,index) in center' :key='index' :offset="{width: -22, height: -50}" v-if='item.count>0&&item.opt==""' @click='chenalert(item.lng+","+item.lat+","+item.count)' :content='item.count+"课"' :position='item' :labelStyle="{backgroundColor:'rgba(0,0,0,0)',backgroundSize:'100% 100%',backgroundImage:'url(http://dev.dianke8.com:7000/xxdk/static/admin/login/cionbg.png)',borderColor:' rgba(0,0,0,0) ',color: '#fff ',textAlign:'center ',lineHeight:'32px ',padding:'5px ',boxSize:'border-box ', fontSize : '12px ',left:'50px',width:'32px ',height:'39px ',top:'-50px '}" title='Hover me' />
			<bm-label v-for='(item,index) in center' :key='index' :offset="{width: -22, height: -50}" v-if='item.count>0&&item.opt!==""' @click='chenalert(item.lng+","+item.lat+","+item.count)' :content='item.opt.substr(5,2)+"<br>"+item.count+"课"' :position='item' :labelStyle="{backgroundColor:'rgba(0,0,0,0)',backgroundSize:'100% 100%',backgroundImage:'url(http://dev.dianke8.com:7000/xxdk/static/admin/login/cionbg.png)',borderColor:' rgba(0,0,0,0) ',color: '#fff ',textAlign:'center ',lineHeight:'16px ',padding:'5px ',boxSize:'border-box', fontSize : '12px ',width:'32px ',height:'39px'}" title='Hover me' />
