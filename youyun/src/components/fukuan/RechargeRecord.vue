			...
			<pull-to :top-load-method="refresh" :bottom-load-method="getData" :bottom-config="{failText: '没有更多信息'}">
				<div class="detail" v-for="(item, index) in listData" :key="index">
					<div class="detail-item">
						<span>充值时间: {{item.created_time}}</span>
					</div>
					<div class="detail-item">
						<p>充值账号: {{item.send_account }}</p>
						<span class="label">{{item.trans_amount}}元</span>
					</div>
					<div class="detail-item">
						<span>到账账户: {{item.sub_account_no}}</span>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.current_page = 1
				this.total_pages = 0
				this.$http.post('account/html/post/v1/recharge_list?token=' + this.token, {
					data: {
					...
					if(res.data.result.reCode === '0') {
						this.listData = res.data.data.recharge_list
			...
			getData(loaded) {
				...
				this.$http.post('account/html/post/v1/recharge_list?token=' + this.token, {
					...
					//					console.log(res)
					if(res.data.result.reCode === '0') {
						this.listData = this.listData.concat(res.data.data.recharge_list)
						this.current_page = this.current_page + 1
						loaded('done')
					}else {
						loaded('fail')
