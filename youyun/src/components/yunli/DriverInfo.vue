<template>
	<div class="driver-info-box">
		...
		<group label-width="85px">
			<x-input title="司机姓名:" placeholder="请输入姓名" :required="true" v-model="realname" :max="16"></x-input>
...
//						this.addressValue = []
					...
					//删除全国选项
					let namearr = []
					for (let i = 0; i < this.direction_name_arry.length; i++) {
						if(this.direction_name_arry[i] !== '全国' && this.direction_name_arry[i] !== '') {
//							this.direction_name_arry.splice(i, 1)
							namearr.push(this.direction_name_arry[i])
						}
					}
					this.direction_name_arry = namearr
					let codearr = []
					for (let j = 0; j < this.direction_code_arry.length; j++) {
						if(this.direction_code_arry[j] !== '4' && this.direction_code_arry[j] !== '') {
//							this.direction_code_arry.splice(j, 1)
							codearr.push(this.direction_code_arry[j])
						}
					}
					this.direction_code_arry = codearr
//					this.addressValue = []
